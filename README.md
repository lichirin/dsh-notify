...
中文名：完成通知dsh插件。仓库名 dsh-notify。

简体中文 | [English](README.en.md)

## 本副本（DeepSeek Harness 0.1.7-rc.1）

功能：一轮对话结束、出错、达成目标时，如果窗口不在前台，就弹出 Windows 通知并播放声音。需要你回答或批准时，即使窗口在前台也会提醒。会话头部有一颗铃铛，用来静音和调音量。
...
装上重启后就可以用，不必先改配置。铃铛在会话右上角。配置写在 `~/.dsh/dsh-notify.json`。来源和许可证见 [来源说明.md](来源说明.md)。
